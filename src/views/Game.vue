<template>
  <div class="game-container">
    <header class="game-header">
      ...
      <div class="progress pill" v-if="questions.length > 0">进度: <span>{{ currentIndex + 1 }}/{{ questions.length }}</span></div>
    ...
    <main class="game-area">
      ...
      <div v-else-if="isGameOver" class="state-view pixel-card end-game">
        ...
        <div class="final-score">最终得分<span>{{ quizStore.currentScore }}</span></div>
        <p class="stats pill">总用时: {{ formatTime(timeSpent) }} | 正确率: {{ quizStore.getCurrentAccuracy() }}%</p>
        <p class="encouragement">{{ getEncouragementText() }}</p>
        <div class="actions">
          <button class="pixel-btn secondary-btn" @click="goBack">返回关卡</button>
          <button v-if="hasNextLevel" class="pixel-btn primary-btn" @click="goToNextLevel">下一关卡</button>
...
function getEncouragementText() { const a = quizStore.getCurrentAccuracy(); if (a === 100) return '太棒了！全部答对！'; if (a >= 70) return '表现不错，继续努力！'; return '失败是成功之母，复习一下吧！' }
function formatTime(s: number) { return `${Math.floor(s/60)}分${s%60}秒` }
function getQuestionById(qId: string) { return questions.value.find(q => String(q.id) === String(qId)) }
function getQuestionPrompt(qId: string) { return getQuestionById(qId)?.prompt || '未知题目' }
function getQuestionAnswer(qId: string) { const q = getQuestionById(qId); if (!q) return ''; if (typeof q.answer === 'boolean') return q.answer ? '正确' : '错误'; return q.answer }
function getQuestionExplanation(qId: string) { return getQuestionById(qId)?.explanation || '暂无解析' }
</script>
